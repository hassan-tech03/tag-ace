...
export default function SearchModal({ isOpen, onClose }) {
  ...
  // Debounced live search against the storefront API. Results are already
  // in storefront shape (numeric price, image URL, gender, fragranceFamily).
  useEffect(() => {
    if (!query.trim()) {
      setResults([]);
      setActiveIdx(-1);
    ...
    const handle = setTimeout(() => {
      fetch(
        `/api/storefront/products?search=${encodeURIComponent(query.trim())}&limit=8`,
        { cache: 'no-store' }
      )
        .then((r) => (r.ok ? r.json() : null))
        .then((b) => {
          if (requestRef.current !== reqId) return;
          setResults(Array.isArray(b?.items) ? b.items : []);
          setActiveIdx(-1);
        })
        .catch(() => {
        ...
        .finally(() => {
          if (requestRef.current === reqId) setSearching(false);
        });
    }, 220);
      ...
      <div className="search-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="search-modal-body">
          ...
          {query && searching && results.length === 0 && (
            <div className="search-no-results" style={{ paddingTop: 24 }}>
              <p className="no-results-title">Searching…</p>
            </div>
          )}
                    ...
                    >
                      ...
                      <div className="result-info">
                        ...
                        <span className="result-meta">
                          {product.gender && (
                            <span className={`result-category result-category--${product.gender}`}>
                              {product.gender}
                            </span>
                          )}
                      ...
                      <div className="result-price">
                        <span className="result-current">${Number(product.price || 0).toFixed(2)}</span>
                        {product.originalPrice > product.price && (
                          <span className="result-original">${Number(product.originalPrice).toFixed(2)}</span>
                        )}
                      </div>
